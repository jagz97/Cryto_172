...
const textToBin = (text) => text.split('').map(char => ("00000000" + char.charCodeAt(0).toString(2)).substr(-8)).join("");
const binToText = (bin) => chunkString(bin, 8).map(byte => String.fromCharCode(parseInt(byte, 2))).join("");
...
const keySchedule = (key) => {
  ...
  return subkeys.map(subkey => chunkString(subkey, 8).map(binToHex).join("")).join("");
};
...
const des = (msg, key, subkeys) => {
  ...
  return chunkString(enc, 8).map(binToHex).join("").toUpperCase();
}

const encode = (msg, key) => des(textToBin(msg), textToBin(key), chunkString(keySchedule(textToBin(key)), 48));
const decode = (msg, key) => binToText(des(textToBin(msg), textToBin(key), chunkString(keySchedule(textToBin(key)), 48).reverse()));

// Example usage
let key = "133457799BBCDFF1"; // hex
let msg = "This is jagjit Singh";
